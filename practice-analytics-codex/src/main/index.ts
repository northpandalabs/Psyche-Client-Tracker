...
ipcMain.handle("export:csv",async()=>{requireUnlocked(unlocked);const date=new Date().toISOString().slice(0,10);const result=await dialog.showSaveDialog({defaultPath:`practice-analytics-${date}.csv`,filters:[{name:"CSV",extensions:["csv"]}]});if(result.canceled||!result.filePath)return false;const rows=database.entries();const header="date,scheduled,new_psych_eval,followup_med,therapy_med,therapy_only,other,cancellations,no_shows,gross_billed_cents,expected_allowed_cents,insurance_paid_cents,patient_paid_cents,other_paid_cents,adjustments_cents,refunds_cents\n";const body=rows.map(x=>[x.date,x.scheduledCount,x.visits.new_psych_eval,x.visits.followup_med,x.visits.therapy_med,x.visits.therapy_only,x.visits.other,x.cancellationCount,x.noShowCount,x.grossBilledCents,x.expectedAllowedCents,x.insurancePaidCents,x.patientPaidCents,x.otherPaidCents,x.adjustmentsCents,x.refundsCents].join(",")).join("\n");await writeFile(result.filePath,header+body,"utf8");return true;});
